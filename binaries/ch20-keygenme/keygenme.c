/*
 * keygenme.c — Programme de vérification de clé de licence (éducatif)
 *
 * Formation Reverse Engineering — Chapitre 20
 * Licence MIT — Usage strictement éducatif
 *
 * Ce binaire implémente une routine de validation de clé basée sur :
 *   - un username transformé par XOR et rotation
 *   - un format de clé XXXXXXXX-XXXXXXXX-XXXXXXXX-XXXXXXXX (hex)
 *   - une comparaison octet par octet
 *
 * Compilé à différents niveaux d'optimisation pour observer l'impact
 * sur le désassemblage et la décompilation.
 */
...
static int parse_key_input(const char *input, uint8_t *out) {
    /* Format attendu : XXXXXXXX-XXXXXXXX-XXXXXXXX-XXXXXXXX (hex) */
...
static int verify_key(const uint8_t *expected, const uint8_t *provided) {
    int result = 0;
    /* Comparaison en temps constant (intentionnellement visible en RE) */
...
static void print_banner(void) {
    ...
    puts("|   Formation Reverse Engineering GCC      |");
    puts("+-----------------------------------------+");
    puts("");
}
...
int main(int argc, char *argv[]) {
    ...
    if (strlen(ctx.username) < 3) {
        fprintf(stderr, "[!] Username trop court (min 3 caracteres).\n");
        return 1;
    }
    ...
    if (parse_key_input(key_input, provided_key) != 0) {
        fprintf(stderr, "[!] Format de cle invalide.\n");
        return 1;
    }

    if (verify_key(ctx.expected_key, provided_key)) {
        puts("[+] Cle valide ! Licence activee.");
        puts("[+] FLAG: RE{k3yg3n_m4st3r_gcc}");
        return 0;
    } else {
        puts("[-] Cle invalide. Reessayez.");
        return 1;
    }
}
