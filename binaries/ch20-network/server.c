/*
 * server.c — Serveur TCP avec protocole binaire custom
 *
 * Formation Reverse Engineering — Chapitre 20
 * Licence MIT — Usage strictement éducatif
 *
 * Écoute sur le port 4337, accepte un client à la fois.
 * Authentification par username + hash, puis exécution de commandes.
 */
...
/* Credentials hardcodés (à retrouver en RE) */
...
static void generate_token(uint8_t *token) {
    /* Token pseudo-aléatoire (volontairement faible pour le RE) */
...
static int recv_message(int fd, uint8_t *type,
    ...
    if (hdr[0] != PROTO_MAGIC_0 || hdr[1] != PROTO_MAGIC_1) {
        fprintf(stderr, "[srv] Magic invalide\n");
        return -1;
    }
    if (hdr[2] != PROTO_VERSION) {
        fprintf(stderr, "[srv] Version non supportee: 0x%02x\n", hdr[2]);
        return -1;
    }
    ...
    *plen = read_be16(&hdr[4]);
    ...
    if (chk_recv != chk_calc) {
        fprintf(stderr, "[srv] Checksum invalide\n");
        return -1;
    }
...
static void handle_auth(int fd, const uint8_t *payload, uint16_t plen) {
    ...
    if (strncmp(req->username, VALID_USER, 32) == 0 &&
        ...
        authenticated = 1;
        printf("[srv] Auth OK pour '%s'\n", req->username);
    } else {
        resp.success = 0;
        printf("[srv] Auth FAIL pour '%.*s'\n", 32, req->username);
    }
...
static void handle_cmd(int fd, const uint8_t *payload, uint16_t plen) {
    ...
    case CMD_GET_INFO: {
        const char *info =
            "KeyGenMe Training Server v1.0 -- RE Formation GCC";
        send_message(fd, MSG_CMD_RESP,
...
static void handle_client(int client_fd) {
    ...
    while (recv_message(client_fd, &type, payload, &plen) == 0) {
        ...
        case MSG_PING:
            ...
            break;
        case MSG_DISCONNECT:
            printf("[srv] Client deconnecte proprement\n");
            return;
        default:
            fprintf(stderr, "[srv] Type inconnu: 0x%02x\n", type);
            break;
        }
    }
    printf("[srv] Client deconnecte\n");
}
...
int main(void) {
    ...
    printf("[srv] Ecoute sur le port %d...\n", DEFAULT_PORT);
