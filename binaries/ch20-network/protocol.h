 /*
 * protocol.h — Définition du protocole custom client/serveur
 *
 ...
 *
 * Types de messages :
 *   0x01 AUTH_REQ   — client envoie username + hash
 *   0x02 AUTH_RESP  — serveur répond OK/FAIL + token
 *   0x03 CMD_REQ    — client envoie une commande
 *   0x04 CMD_RESP   — serveur répond avec les données
 *   0x05 PING       — keepalive
 *   0x06 PONG       — réponse keepalive
 *   0xFF DISCONNECT — fin de session
 */
...
/* Header du protocole (sérialisé en big-endian sur le réseau) */
...
typedef struct __attribute__((packed)) {
    char    username[32];
    uint8_t password_hash[PROTO_HASH_LEN]; /* hash simplifié */
} auth_req_payload_t;

/* Payload AUTH_RESP */
typedef struct __attribute__((packed)) {
    uint8_t  success;                  /* 0x00 = fail, 0x01 = success */
    uint8_t  token[PROTO_TOKEN_LEN];   /* session token si success */
} auth_resp_payload_t;
...
/* --- Fonctions utilitaires du protocole --- */
