/*
 * client.c — Client TCP pour le protocole custom
 *
 * Formation Reverse Engineering — Chapitre 20
 * Licence MIT — Usage strictement éducatif
 *
 * Se connecte au serveur, s'authentifie et envoie des commandes.
 * Les étudiants doivent reverse-engineerer le protocole à partir
 * du binaire serveur seul, puis écrire leur propre client.
 */
...
/* Hash simplifié du mot de passe (à retrouver en RE) */
...
int main(int argc, char *argv[]) {
    ...
    if (recv_message(fd, &resp_type, resp_payload, &resp_len) == 0) {
        ...
        printf("[cli] Info serveur: %s\n", (char *)resp_payload);
    }

    /* --- Envoyer CMD_LIST_FILES --- */
    ...
    /* --- Envoyer CMD_READ_FILE --- */
    cmd->cmd_id = CMD_READ_FILE;
    send_message(fd, MSG_CMD_REQ, cmd_buf, sizeof(cmd_req_header_t));

    if (recv_message(fd, &resp_type, resp_payload, &resp_len) == 0) {
        resp_payload[resp_len] = '\0';
        printf("[cli] Contenu: %s", (char *)resp_payload);
    }

    /* --- Ping --- */
    send_message(fd, MSG_PING, NULL, 0);
    if (recv_message(fd, &resp_type, resp_payload, &resp_len) == 0) {
        printf("[cli] Pong recu !\n");
    }

    /* --- Déconnexion --- */
    send_message(fd, MSG_DISCONNECT, NULL, 0);
    printf("[cli] Deconnecte.\n");
