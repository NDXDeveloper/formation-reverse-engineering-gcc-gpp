 ...
 *
 * Ce plugin applique un chiffrement ROT13 sur les caractères alphabétiques.
 * Il est chargé dynamiquement par l'application principale via dlopen/dlsym.
 *
 * Symboles exportés (extern "C") :
 *   - Processor* create_processor(uint32_t id)
 *   - void destroy_processor(Processor* p)
 *
 * En RE, ce .so sera analysé séparément. L'étudiant devra :
 *   1. Identifier les symboles factory avec nm -CD
 *   2. Retrouver la classe Rot13Processor et sa vtable
 *   3. Comprendre que cette classe hérite de Processor (même interface)
 */
...
public:
    ...
    bool configure(const char* key, const char* value) override {
        /* "half_rot" → ROT6 au lieu de ROT13 (pour varier) */
...
/* === Fonctions factory (extern "C" pour éviter le mangling) === */
