 ...
 * Client TCP pour le protocole binaire custom
 ...
 *   port     : port TCP (défaut: 4444)
 *   username : identifiant (défaut: admin)
 *   password : mot de passe (défaut: s3cur3P@ss!)
 */
...
static int do_handshake(int fd, uint8_t *challenge_out)
{
    ...
    /* HELLO Request : "HELLO" + 3 octets de padding */
                   ...
                   const uint8_t *challenge)
{
    ...
    /* XOR le mot de passe avec le challenge */
...
static void print_file_list(const uint8_t *data, uint16_t len)
{
    ...
    /* data[0] = STATUS_OK (déjà vérifié), data[1] = count */
    /* On reçoit data à partir de resp+1 */
                            ...
                            const char *password)
{
    ...
    /* LIST — recevoir et afficher la liste */
    printf("\n[*] CMD: LIST\n");
    {
