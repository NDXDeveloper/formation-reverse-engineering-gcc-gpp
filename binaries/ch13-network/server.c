 ...
 *
 * Serveur d'entraînement utilisant le protocole custom GCRP.
 * Écoute sur le port 4444, gère un client à la fois.
 *
...
/* ═══════════════════════════════════════════════
 * Données de ressources (réponses aux MSG_DATA_REQ)
 * ═══════════════════════════════════════════════ */

static const char *resources[] = {
    [0] = "FLAG{fr1da_h00k5_ar3_p0w3rful}",
    [1] = "Ceci est la ressource confidentielle n°1.",
    [2] = "AES-256-CBC key: 0xDEADBEEF (fake, pour l'exercice).",
    [3] = "Données binaires simulées pour le reverse engineering.",
};
...
/**
 * Envoie exactement `len` octets sur le socket.
 * Gère les envois partiels (short writes).
 */
...
/**
 * Reçoit exactement `len` octets depuis le socket.
 */
...
/**
 * Envoie un paquet GCRP complet (header + payload).
 */
...
/**
 * Reçoit un paquet GCRP. Remplit `type`, `payload` et `payload_len`.
...
static int recv_packet(int sock, uint8_t *type,
    ...
    if (memcmp(hdr.magic, GCRP_MAGIC, 4) != 0) {
        fprintf(stderr, "[!] Magic invalide : %02x %02x %02x %02x\n",
...
static int handle_auth(int sock, const void *payload, uint16_t len) {
    if (len < sizeof(auth_req_payload_t)) {
        fprintf(stderr, "[!] AUTH_REQ payload trop petit (%u)\n", len);
        return -1;
    }
    ...
    if (ulen == 0 || !verify_token(req->username, req->token, ulen)) {
        ...
        /* Générer un session ID pseudo-aléatoire */
...
static int handle_data_req(int sock, const void *payload, uint16_t len) {
    ...
    /* Envoyer : resource_id(1) + data_len(2) + data(clen) */
...
static void handle_client(int client_sock, struct sockaddr_in *client_addr) {
    ...
    while (1) {
        if (recv_packet(client_sock, &type, payload, &payload_len) < 0) {
            printf("[-] Connexion perdue ou erreur de protocole.\n");
            break;
        }
        ...
        switch (type) {
            ...
            case MSG_GOODBYE:
                printf("[*] GOODBYE reçu. Fermeture de la session.\n");
                goto done;

            default:
                fprintf(stderr, "[!] Type de message inconnu : 0x%02x\n", type);
